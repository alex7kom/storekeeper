  ...
  function (callback) {
    ...
    sk.steam.on('webSessionID', function (sessionID) {
      sk.steam.webLogOn(function (newCookie) {
        sk.tradeOffers.setup(sessionID, newCookie, function (error) {
          if (error) {
            throw new Error('This account is limited. You need to own at least one game. Check https://support.steampowered.com/kb_article.php?ref=3330-IAGK-7663 for details.');
          }
        });
      ...
      for (x in appData.rpcMethods) {
        appData.rpcMethods[x].forEach(function (method) {
          ...
          if (config.methodsWhitelist) {
            if (config.methodsWhitelist.indexOf(methodName) != -1) {
              add = true;
            }
          } else {
            if (!config.methodsBlacklist || config.methodsBlacklist.indexOf(methodName) == -1) {
              add = true;
            }
          }
          if (add) {
    ...
    function startClient() {
      ...
      for (x in appData.rpcEvents){
        appData.rpcEvents[x].forEach(function (event) {
          var eventName = x + '.' + event;
          var listen = false;
          if (config.eventsWhitelist) {
            if (config.eventsWhitelist.indexOf(eventName) != -1) {
              listen = true;
            }
          } else {
            if (!config.eventsBlacklist || config.eventsBlacklist.indexOf(eventName) == -1) {
              listen = true;
            }
          }
          if (listen) {
